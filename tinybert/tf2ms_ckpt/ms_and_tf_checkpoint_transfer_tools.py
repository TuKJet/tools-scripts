...
def convert_ms_2_tf(tf_ckpt_path, ms_ckpt_path, new_ckpt_path):
    ...
    with tf.Session() as sess:
        ...
        new_var_list = []
        for var_name, shape in tf.contrib.framework.list_variables(
                tf_ckpt_path):
            if var_name in ms2tf_param_dict:
                ...
                if len(shape) == 2:
                    if tuple(
                            shape) != new_tensor.shape or new_tensor.shape[0] == new_tensor.shape[1]:
                ...
                var = tf.Variable(
                    tf.contrib.framework.load_variable(
                        tf_ckpt_path, var_name), name=var_name)
            new_var_list.append(var)
        print(
            'convert value num: ',
            convert_count,
            " of ",
            len(ms2tf_param_dict))
...
def convert_tf_2_ms(tf_ckpt_path, ms_ckpt_path, new_ckpt_path):
    ...
    """
    tf2ms_param_dict = dict(
        zip(ms2tf_param_dict.values(), ms2tf_param_dict.keys()))
...
def main():
    ...
    parser.add_argument("--tf_ckpt_path", type=str, default='/home/admin/dataset/uncased_L-12_H-768_A-12/bert_model.ckpt',
                        help="TensorFlow checkpoint dir, default is: './tf-bert/bs64k_32k_ckpt_model.ckpt-28252'.")
    parser.add_argument("--ms_ckpt_path", type=str, default='/home/admin/dataset/uncased_L-12_H-768_A-12/bert_base_uncased.ckpt',
                        help="MindSpore checkpoint dir, default is: '/home/admin/dataset/cased_L-12_H-768_A-12/bert_base.ckpt'.")
    parser.add_argument("--new_ckpt_path", type=str, default='./bert_base_uncased.ckpt',
                        help="New checkpoint dir, default is: './new_ckpt/new_bert_large_en.ckpt'.")
    parser.add_argument("--transfer_option", type=str, default='tf2ms',
